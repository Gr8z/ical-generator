...
var ICalTools = /*#__PURE__*/function () {
  ...
  _createClass(ICalTools, null, [{
    ...
    key: "formatVTZ",
    value: function formatVTZ(timezone) {
      var zone = moment.tz.zone(timezone);
      var g = '';

      for (var i = 0; i < 2 && i + 1 < zone.untils.length; i++) {
        var type = i % 2 == 0 ? 'STANDARD' : 'DAYLIGHT';
        var momDtStart = moment.tz(zone.untils[i], timezone);
        var momNext = moment.tz(zone.untils[i + 1], timezone);
